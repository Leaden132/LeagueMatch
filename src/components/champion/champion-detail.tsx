...
export function ChampionDetail({ champion, version }: ChampionDetailProps) {
  ...
  const isDuplicate = favorites?.some((f) => f.champ_name === champion.id);
      ...
      <div className={`wrapper ${styles.content}`}>
        <div className={styles.header}>
          ...
          <div>
            ...
            <p className={styles.tags}>{champion.tags.join(" / ")}</p>
          </div>
          ...
          {user && (
            <Button
              ...
              size="sm"
              onClick={handleAdd}
              disabled={isDuplicate || addFavorite.isPending}
              className={styles.addBtn}
            >
              {isDuplicate ? "Already in list" : `Add ${champion.name}`}
            </Button>
          )}
        </div>
        ...
        <div className={styles.abilities}>
          ...
          {champion.spells.map((spell, i) => (
            ...
            >
              <div className={styles.ability}>
                <img
                  src={spellImageUrl(version, spell.image.full)}
                  alt={spell.name}
                />
